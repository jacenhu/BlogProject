...
802.4 针对令牌总线网络
802.5 针对令牌环网络
![http-ip-005](/img/httpip/httpip_005.png)
...
![http-ip-006](/img/httpip/httpip_006.png)
...
![http-ip-007](/img/httpip/httpip_007.png)

![http-ip-008](/img/httpip/httpip_008.png)
...
![http-ip-009](/img/httpip/httpip_009.png)
...
![http-ip-010](/img/httpip/httpip_010.png)
...
ifconfig eth0
```
![http-ip-011](/img/httpip/httpip_011.png)

### 3.9 netstat 命令
netstat命令也提供系统上的接口信息。-i参数将打印出接口信息，-n参数则打印出IP地址，而不是主机名字。

![http-ip-012](/img/httpip/httpip_012.png)
...
![http-ip-013](/img/httpip/httpip_013.png)
### 4.2 一个例子
...
arp -a  // 显示所有高速缓存中的内容
```
![http-ip-014](/img/httpip/httpip_014.png)

### 4.4 ARP的分组格式

![http-ip-015](/img/httpip/httpip_015.png)

对一个ARP请求来说，除目的端硬件地址外的所有其他的字段都有填充值。

![http-ip-016](/img/httpip/httpip_016.png)
...
### 5.3 RARP举例
当无盘系统从RARP应答中收到它的IP地址后，它将发送 TFTP请求来读取引导映象。
### 5.4 RARP服务器的设计
...
### 7.2 Ping程序
ICMP回显请求和回显应答报文格式：
![http-ip-017](/img/httpip/httpip_017.png)
...
![http-ip-018](/img/httpip/httpip_018.png)
### 7.3 IP记录路由选项
-R： 记录路由
![http-ip-019](/img/httpip/httpip_019.png)
![http-ip-020](/img/httpip/httpip_020.png)
### 7.4 IP时间戳选项
![http-ip-021](/img/httpip/httpip_021.png)
### 7.5 小结
...
TCP的可靠性保证：
![http-ip-022](/img/httpip/httpip_022.png)

### 17.3 TCP的首部
TCP数据被封装在一个IP数据报中。
![http-ip-001](/img/httpip/httpip_001.png)
TCP首部通常是20字节。
![http-ip-002](/img/httpip/httpip_002.png)
...
### 18.2 连接的建立与终止
![httpip_023](/img/httpip/httpip_023.png)
源>目的：标志
win表示发端通告的窗口大小
mss表示发端指明的最大报文段长度选项
...
![httpip_003](/img/httpip/httpip_003.png)
...
![image-20201109161904953](/img/httpip/httpip_004.png)
### 18.3 连接建立的超时
### 18.4 最大报文段长度
...
### 18.6 TCP的状态变迁图
![httpip_025](/img/httpip/httpip_025.png)
**2MSL等待时间**：
TIME_WAIT状态也称为2MSL等待状态。每个具体TCP实现必须选择一个报文段最大生存时间MSL（Maximum Segment Lifetime）。它是任何报文段被丢弃前在网络内的最长时间。
### 18.7 复位报文段
RST用于复位。
### 18.8 同时打开
TCP是特意设计为了可以处理同时打开，对于同时打开它仅建立一条连接而不是两条连接（其他的协议族，最突出的是 OSI运输层，在这种情况下将建立两条连接而不是一条连接）。
![httpip_026](/img/httpip/httpip_026.png)
### 18.9 同时关闭
![httpip_027](/img/httpip/httpip_027.png)
### 18.10 TCP选项
### 18.11 TCP服务的设计
### 18.12 小结
...
* 在较慢的广域网环境中，通常使用Nagle算法减少小报文段的数量。这个算法限制发送者任何时候只能有一个发送的小报文段未被确认。
...
**PUSH标志**：
发送方使用
该标志通知接收方将所收到的数据全部提交给接收进程。这里的数据包括与PUSH一起传送的数据以及接收方TCP已经为接收进程收到的其他数据。
...
URG比特被置1，并且一个 16bit的紧急指针被置为一个正的偏移量，该偏移量必须与TCP首部中的序号字段相加，以便得出紧急数据的最后一个字节的序号。
...
TCP通过在发送时设置一个定时器来解决这种问题。如果当定时器溢出
时还没有收到确认，它就重传该数据。

### 21.3 往返时间测量

![httpip_028](/img/httpip/httpip_028.png)
...
拥塞避免算法和慢启动算法需要对每个连接维持两个变量：一个拥塞窗口 cwnd和一个慢启动门限ssthresh。P235

![httpip_029](/img/httpip/httpip_029.png)

### 21.7 快速重传与快速恢复算法

算法步骤：
1) 当收到第3个重复的ACK时，将ssthresh设置为当前拥塞窗口cwnd的一半。重传丢失的
报文段。设置cwnd为ssthresh加上3倍的报文段大小。
